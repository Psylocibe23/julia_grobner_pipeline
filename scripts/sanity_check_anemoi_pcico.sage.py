...
# This file was *autogenerated* from the file scripts/sanity_check_anemoi_pcico.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_500 = Integer(500); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_50000 = Integer(50000)###############################################################################
...
args = {
    ...
    "alpha": None,
    "rounds": None,
    "tries": _sage_const_500 ,
    "exhaustive": False,
    "max_sol": _sage_const_2 ,
    "verbose": False,
}
i = _sage_const_0 
while i < len(argv):
    a = argv[i]
    if a == "--infile" and i+_sage_const_1  < len(argv):
        args["infile"] = argv[i+_sage_const_1 ]; i += _sage_const_2 
    ...
    elif a == "--tries" and i+_sage_const_1  < len(argv):
        args["tries"] = int(argv[i+_sage_const_1 ]); i += _sage_const_2 
    elif a == "--exhaustive":
        args["exhaustive"] = True; i += _sage_const_1 
    elif a == "--max-sol" and i+_sage_const_1  < len(argv):
        args["max_sol"] = int(argv[i+_sage_const_1 ]); i += _sage_const_2 
    elif a == "--verbose":
        args["verbose"] = True; i += _sage_const_1 
    else:
        usage()
...
G_model, _ = model_P_CICO(P, n_rounds=r, final_ll=True, ordering=_sage_const_1 , debug=False)
...
def find_assignments(max_needed=_sage_const_2 , exhaustive=False, tries=_sage_const_500 , verbose=False):
    sols = []
    # generator of y0 candidates
    if exhaustive or p <= _sage_const_50000 :
        cand = list(F)
    else:
    ...
    for y0 in cand:
        ...
        y = F(y0)
        s_values = []
        for t in range(_sage_const_1 , r+_sage_const_1 ):
            # add round constants (r-1 index in code)
            x_in = x + P.C[t-_sage_const_1 ][_sage_const_0 ]
            y_in = y + P.D[t-_sage_const_1 ][_sage_const_0 ]
            # apply linear layer
        ...
        for t, sval in enumerate(s_values, start=_sage_const_1 ):
            vname = f"S{t:02d}00"
        ...
        ok = all(g.subs(assignment) == _sage_const_0  for g in G_file)
        if ok:
...
exhaustive = args["exhaustive"] or (p <= _sage_const_50000 )
...
if len(sols) == _sage_const_0 :
    ...
    y0, svals, assign = sols[_sage_const_0 ]
    ...
    print(f"    Y0000 = {pv(y0)}")
    for t, sval in enumerate(svals, start=_sage_const_1 ):
