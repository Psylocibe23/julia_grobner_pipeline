...
_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_20 = Integer(20)
import sys
import time
import os
    ...
    with open(filename) as f:
        for line in f:
            if line.startswith("# Variables:"):
                vars_line = line.strip().split(":",_sage_const_1 )[_sage_const_1 ].strip()
            if line.startswith("# Field characteristic:"):
                char_line = line.strip().split(":",_sage_const_1 )[_sage_const_1 ].strip()
            if line.startswith("# Field: GF("):
                gf_line = line.strip().split("GF(",_sage_const_1 )[_sage_const_1 ].split(")")[_sage_const_0 ].strip()
    if vars_line:
...
def main():
    ...
    base_name = os.path.splitext(os.path.basename(basisfile))[_sage_const_0 ]
    results_dir = "results"
    logs_dir = "logs"
    ...
    try:
        ...
        for vals in itertools.product(F, repeat=len(vars)):
            ...
            if all(poly(**subst) == _sage_const_0  for poly in polynomials):
                sols.append(subst)
        method = "brute_force"
    t1 = time.time()
    solve_time = t1 - t0
    ...
    for sol in sols:
        ...
        for poly in polynomials:
            if not poly(**strsol) == _sage_const_0 :
                all_verified = False
