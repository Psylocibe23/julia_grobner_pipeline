...
_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_60 = Integer(60); _sage_const_10 = Integer(10); _sage_const_5 = Integer(5)###############################################################################
    ...
    for i, line in enumerate(lines):
        ...
        if s.startswith("# Variables:"):
            ...
            try:
                ...
                field_p = int(inside.split("^", _sage_const_1 )[_sage_const_0 ].strip())
            except Exception:
    ...
    with open(log_outfile, "w") as log:
        ...
        try:
            ...
            try:
                ...
                log_and_print(f"Degrees(in): max={max(degs)}, list(first 10)={degs[:_sage_const_10 ]}{'...' if len(degs)>_sage_const_10  else ''}", log)
            ...
            if reduce_mode_norm == "never":
                ...
                try:
                    G_lex = run_with_timeout(_reduce, reduce_timeout if reduce_mode_norm in ("auto","always") else _sage_const_0 )
                except _Timeout:
            ...
            log_and_print(f"Degrees(out): max={max(out_degs)}, list(first 10)={out_degs[:_sage_const_10 ]}{'...' if len(out_degs)>_sage_const_10  else ''}", log)
