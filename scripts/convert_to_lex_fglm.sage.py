...
def is_shape_position(G_lex):
    ...
    try:
        return all(len(g.lm().variables()) == _sage_const_1  for g in G_lex)
    except Exception:
        return False
...
def main():
    ...
    base_name = os.path.splitext(os.path.basename(result_file))[_sage_const_0 ]
    results_dir = "results"
    logs_dir = "logs"
    ...
    # FGLM and stats
    t0 = time.time()
    G_lex_fglm = I_lex.groebner_basis()
    t1 = time.time()
    fglm_time = t1 - t0
    ...
    zero_dim = None
    try:
        zero_dim = (I_lex.dimension() == _sage_const_0 )
        if zero_dim:
            sols = I_lex.variety()
