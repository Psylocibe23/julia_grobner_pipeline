...
# This file was *autogenerated* from the file scripts/expand_field_extension_to_base.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2)#!/usr/bin/env sage
import sys
import os
...
def expand_system(var_names, p, n, polys):
    ...
    for poly_s in polys:
        ...
        coord_polys = [PR_p(_sage_const_0 ) for _ in range(n)]
            ...
            for i, c in enumerate(cvec):
                if c != _sage_const_0 :
                    ...
                    if mono_terms:
                        ...
                        exprstr = f"{c}*{monostr}" if c != _sage_const_1  else monostr
                    else:
                        exprstr = f"{c}"
...
def main():
    ...
    print(f"Parsed: Variables: {var_names}, Field: GF({p}^{n})" if n > _sage_const_1  else f"Field: GF({p})")
    ...
    out_name = os.path.splitext(os.path.basename(infile))[_sage_const_0 ] + "_expanded.in"
    out_path = os.path.join("data", out_name)
    write_output_file(new_var_names, p, _sage_const_1 , new_polys, out_path)
