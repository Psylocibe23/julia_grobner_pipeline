...
# This file was *autogenerated* from the file scripts/verify_quotient_space_dimension.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0)#!/usr/bin/env sage
...
def parse_header(lines):
    ...
    for s in lines:
        ...
        if s.startswith("# Variables:"):
            meta["variables"] = [v.strip() for v in s.split(":",_sage_const_1 )[_sage_const_1 ].split(",")]
        elif s.startswith("# Field:"):
            meta["field"] = s.split(":",_sage_const_1 )[_sage_const_1 ].strip()
        elif s.startswith("# Order:"):
            meta["order"] = s.split(":",_sage_const_1 )[_sage_const_1 ].strip()
        elif s.startswith("# Shape position:"):
            meta["shape_position"] = (s.split(":",_sage_const_1 )[_sage_const_1 ].strip().lower()=="true")
        elif s.startswith("# Zero-dimensional:"):
            meta["zero_dim"] = (s.split(":",_sage_const_1 )[_sage_const_1 ].strip().lower()=="true")
    return meta
...
def build_ring(meta):
    ...
    if fld.startswith("GF("):
        ...
        K = GF(int(m.group(_sage_const_1 )))
    elif fld in ("QQ","Q","RATIONALS"):
        K = QQ
    else:
        m = re.search(r"(\d+)", fld)
        K = GF(int(m.group(_sage_const_1 ))) if m else GF(_sage_const_2 )
...
def triangular_product_if_any(G):
    ...
    for g in G:
        lm = g.lm()
        degs = lm.degrees()
        nz = [(i,e) for i,e in enumerate(degs) if e!=_sage_const_0 ]
        if len(nz)!=_sage_const_1 :
            return (False, {}, None)
        i,e = nz[_sage_const_0 ]
        if i in exps and exps[i]!=e:
            return (False, {}, None)
        exps[i] = e
    prod = Integer(_sage_const_1 )
    for i in exps:
        prod *= exps[i]
...
def main():
    ...
    K = R.base_ring()
    char = _sage_const_0  if not K.is_finite() else K.characteristic()
