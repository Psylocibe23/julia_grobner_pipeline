...
def pick_workers(cli_workers=None, max_secret_tries=2048, batch_size=256, n=0):
    """
    ...
    if cli_workers is not None and not _is_unset(cli_workers):
        try:
            return max(1, min(int(cli_workers), cap_by_batches))
        except Exception:
            pass
    ...
    if env:
        try:
            return max(1, min(int(env), cap_by_batches))
        except Exception:
            pass

    try:
        return max(1, min(os.cpu_count() or 1, cap_by_batches))
    except Exception:
        return 1
        ...
        for amap in range(1, max_maps+1):
            ...
            if W <= 1:
                ...
                with ProcessPoolExecutor(max_workers=W, mp_context=ctx) as ex:
                    futures = set()
                    seed_base = int(time.time()) ^ pyrand.getrandbits(32)
...
def main():
    ...
    if len(sys.argv) >= 4:
        ...
        if _is_unset(a3):
            ...
            out_infile = auto_out; arg_offset = 3  # legacy: treat as seed
        else:
