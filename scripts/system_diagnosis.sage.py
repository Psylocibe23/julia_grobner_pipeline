...
_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_3600 = Integer(3600); _sage_const_60 = Integer(60)###############################################################################
...
def parse_time_budget(tok):
    ...
    if not m:
        ...
        try:
            ...
            return _sage_const_0 
    h = int(m.group(_sage_const_1 ) or _sage_const_0 )
    m_ = int(m.group(_sage_const_2 ) or _sage_const_0 )
    s_ = int(m.group(_sage_const_3 ) or _sage_const_0 )
    return h*_sage_const_3600  + m_*_sage_const_60  + s_
...
def main():
    ...
    infile = None
    for a in sys.argv[_sage_const_1 :]:
        if not a.startswith("--"):
            infile = a
            break
    if infile is None:
    ...
    # Budgets
    dim_budget     = parse_time_budget(get_flag(sys.argv[_sage_const_1 :], "dim", "0"))
    hilbert_budget = parse_time_budget(get_flag(sys.argv[_sage_const_1 :], "hilbert", "0"))
    ...
    if hilbert_budget > _sage_const_0 :
